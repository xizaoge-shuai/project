...
def main():
    ...
    ap.add_argument("--current_total_support_threshold", type=int, default=2)
    ...
    for r in rows:
        ...
        base_pred = clean(r.get("seedaware_answer", cur))
        support = {clean(k): int(v) for k, v in r.get("seedaware_total_support", {}).items()}

        pred = base_pred
        reason = r.get("seedaware_reason", "seedaware_base")

        if support.get(cur, 0) >= args.current_total_support_threshold:
            pred = cur
            reason = f"current_total_support>={args.current_total_support_threshold}_keep"
        ...
        rr = dict(r)
        rr["final_guard_answer"] = pred
        rr["final_guard_reason"] = reason
        rr["final_guard_ok"] = int(pred_ok)
        rr["final_guard_fixed"] = fixed_i
        rr["final_guard_broken"] = broken_i
        rr["final_guard_changed"] = changed_i
        rr["current_total_support"] = support.get(cur, 0)
        out.append(rr)

    net = fixed - broken
    summary = {
        "rule": f"current_total_support>={args.current_total_support_threshold}_keep",
