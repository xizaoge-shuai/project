...
def main():
    ...
    trigger_progress_all: List[float] = []
    trigger_progress_bad: List[float] = []
    trigger_progress_good: List[float] = []
    ...
    for tid in common_ids:
        ...
        if trigger_idx is not None:
            triggered += 1
            trigger_progress_all.append(trigger_progress)
            if is_bad:
                trigger_progress_bad.append(trigger_progress)
            else:
                trigger_progress_good.append(trigger_progress)

            # backtrack recovery proxy:
            # 触发后向后看几个 prefix，
            # 如果很快回到高于 tau_recover，就视作“这次低分可修复”
            upper = min(len(rows), trigger_idx + 1 + args.lookahead_prefixes)
            for j in range(trigger_idx + 1, upper):
                p_next = float(rows[j]["success_prob"])
                if p_next >= args.tau_recover:
                    recovered_this = True
                    break
    ...
    result = {
        ...
        "trigger_rate": triggered / n_total if n_total > 0 else None,
        "num_recovered": recovered,
        "num_recovered_bad": recovered_bad,
        "num_recovered_good": recovered_good,
        "recover_rate_all": recovered / triggered if triggered > 0 else None,
        ...
        "avg_trigger_progress_all": mean_or_none(trigger_progress_all),
        "avg_trigger_progress_bad": mean_or_none(trigger_progress_bad),
        "avg_trigger_progress_good": mean_or_none(trigger_progress_good),
