...
mkdir -p outputs/logs/model_swap_fullroute/deepseek7b/true_ablation
mkdir -p outputs/metrics/model_swap_fullroute/deepseek7b/true_ablation
mkdir -p outputs/predictions/model_swap_fullroute/deepseek7b/true_ablation
mkdir -p outputs/logs/model_swap_fullroute/deepseek7b/final_summaries
...
trap 'echo "========== backup true ablation =========="; \
  tar --ignore-failed-read -czf /root/autodl-tmp/pce_backups/true_generation_ablation_qwen_$(date +%Y%m%d_%H%M%S).tar.gz \
    outputs/logs/model_swap_fullroute/deepseek7b/true_ablation \
    outputs/metrics/model_swap_fullroute/deepseek7b/true_ablation \
    outputs/predictions/model_swap_fullroute/deepseek7b/true_ablation \
    outputs/logs/model_swap_fullroute/deepseek7b/final_summaries/true_generation_ablation_qwen_summary.md || true; \
...
python experiments/analyze_resampling_cost_accuracy.py \
  --n_samples 1319 \
  --out_json outputs/metrics/model_swap_fullroute/deepseek7b/resampling_cost_accuracy_summary.json \
  --out_md outputs/logs/model_swap_fullroute/deepseek7b/final_summaries/resampling_cost_accuracy_summary.md \
  2>&1 | tee outputs/logs/model_swap_fullroute/deepseek7b/true_ablation/analyze_gsm8k_cost_accuracy.log || true
...
ASDIV_EXTRA_FILES="data/processed/trajectories/model_swap_fullroute/deepseek7b/asdiv/extra_numeric_full_seed42.jsonl data/processed/trajectories/model_swap_fullroute/deepseek7b/asdiv/extra_numeric_full_seed101.jsonl data/processed/trajectories/model_swap_fullroute/deepseek7b/asdiv/extra_numeric_full_seed202.jsonl"
    ...
    do
      ...
      do
        python experiments/apply_asdiv_numeric_budget_confirm.py \
          --baseline_details outputs/predictions/model_swap_fullroute/deepseek7b/asdiv_full2305_numeric_baseline_details.jsonl \
          --target_ids outputs/targets/model_swap_fullroute/deepseek7b/asdiv_full2305_numeric_has_disagreement_ids.txt \
          --extra_jsonls $ASDIV_EXTRA_FILES \
          --per_seed_budget "$BUDGET" \
          --min_total_support "$TOTAL" \
          --min_seed_support "$SEEDSUP" \
          --min_margin "$MARGIN" \
          --out_json outputs/metrics/model_swap_fullroute/deepseek7b/true_ablation/asdiv_budget${BUDGET}_total${TOTAL}_seed${SEEDSUP}_margin${MARGIN}.json \
          --out_jsonl outputs/predictions/model_swap_fullroute/deepseek7b/true_ablation/asdiv_budget${BUDGET}_total${TOTAL}_seed${SEEDSUP}_margin${MARGIN}.jsonl
      done
    done
  done
done
...
if [ "$RUN_ASDIV_EXTRA8" = "1" ]; then
  ...
  do
    OUT=data/processed/trajectories/model_swap_fullroute/deepseek7b/asdiv/extra_numeric_full_n8_seed${SEED}.jsonl
    ROWS=0
    ...
    if [ "$ROWS" = "$EXPECTED" ]; then
      ...
      python scripts/generate_numeric_trajectories_local.py \
        ...
        --top_p 0.95 \
        --seed "$SEED" \
        2>&1 | tee outputs/logs/model_swap_fullroute/deepseek7b/true_ablation/generate_asdiv_extra8_seed${SEED}.log
    fi
  done
fi
...
do
  ...
  do
    SEED=${ALL_SEEDS[$i]}
    EXTRA_FILES="$EXTRA_FILES data/processed/trajectories/model_swap_fullroute/deepseek7b/bbh_logic/${TASK}_extra_fixed_has_disagreement_seed${SEED}.jsonl"
  done

  BASE_ACC=$(python - <<PY
import json
x=json.load(open("outputs/metrics/model_swap_fullroute/deepseek7b/bbh_logic_${TASK}_smoke100_baseline_fixed.json", encoding="utf-8"))
    ...
    do
      ...
      do
        python experiments/apply_bbh_fixed_resample_confirm.py \
          --baseline_fixed_details outputs/predictions/model_swap_fullroute/deepseek7b/bbh_logic_${TASK}_smoke100_baseline_fixed_details.jsonl \
          --extra_jsonls $EXTRA_FILES \
          --target_ids outputs/targets/model_swap_fullroute/deepseek7b/bbh_logic_${TASK}_fixed_has_disagreement_ids.txt \
          --subtask "$TASK" \
          --min_total_support "$TOTAL" \
          --min_seed_support "$SEEDSUP" \
          --min_margin "$MARGIN" \
          --base_acc "$BASE_ACC" \
          --n_samples 100 \
          --out_json outputs/metrics/model_swap_fullroute/deepseek7b/true_ablation/bbh_${TASK}_seedbudget${SEED_BUDGET}_total${TOTAL}_seed${SEEDSUP}_margin${MARGIN}.json \
          --out_jsonl outputs/predictions/model_swap_fullroute/deepseek7b/true_ablation/bbh_${TASK}_seedbudget${SEED_BUDGET}_total${TOTAL}_seed${SEEDSUP}_margin${MARGIN}.jsonl || true
      done
    done
  done
done
...
python - <<'PY' | tee outputs/logs/model_swap_fullroute/deepseek7b/final_summaries/true_generation_ablation_qwen_summary.md
import json
from pathlib import Path
...
fp = Path("outputs/logs/model_swap_fullroute/deepseek7b/final_summaries/resampling_cost_accuracy_summary.md")
if fp.exists():
...
print("|---:|---:|---:|---:|---:|---:|---|")
for b in [1,2,4]:
    rows = load_rows(f"outputs/metrics/model_swap_fullroute/deepseek7b/true_ablation/asdiv_budget{b}_*.json")
...
print("|---:|---:|---:|---:|---:|---:|---|")
for sb in [3,6,12]:
    rows = load_rows(f"outputs/metrics/model_swap_fullroute/deepseek7b/true_ablation/bbh_logical_deduction_five_objects_seedbudget{sb}_*.json")
