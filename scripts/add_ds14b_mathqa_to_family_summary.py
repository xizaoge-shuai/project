...
IN = Path("outputs/metrics/baseline_tts_compare_mathqa_14b/mathqa_ds14b_optionmap_fair_fullbase_fixed_baseline_compare.csv")
if not IN.exists():
    IN = Path("outputs/metrics/baseline_tts_compare_mathqa_14b/mathqa_ds14b_optionmap_fair_baseline_compare.csv")

OUT = Path("outputs/metrics/baseline_14b_compare/ds14b_family_summary_with_mathqa.md")
...
def cell(family):
    ...
    pos_harm = [x for x in cr if x["harm"] is not None and (x["acc"] or 0) > 0]
    best_harm = min(pos_harm, key=lambda x:x["harm"], default=None)
...
lines = []
old = Path("outputs/metrics/baseline_14b_compare/ds14b_family_summary.md")
if old.exists():
    lines = old.read_text(encoding="utf-8").splitlines()
else:
    lines = ["# DS14B Family Summary", "", "Cell format: `Final Acc / Cost-Acc / Repair-P / Harm`.", "", "| Dataset | SC | ESC | CISC | GG-lite-proxy | Ours |", "|---|---|---|---|---|---|"]

# 删除旧 mathqa 行
lines = [ln for ln in lines if not ln.startswith("| mathqa |")]

lines.append(f"| mathqa | {cell('SC')} | {cell('ESC')} | {cell('CISC')} | — | {cell('Ours')} |")
