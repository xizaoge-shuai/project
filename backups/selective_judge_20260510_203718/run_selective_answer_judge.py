...
def build_judge_prompt(question: str, candidates: List[Dict[str, Any]], ranked: List[Tuple[str, float]]) -> str:
    lines = []
...
def _normalize_generation_output(out: Any) -> str:
    """Normalize outputs from different local generator wrappers."""
    if out is None:
        return ""
    ...
    if isinstance(out, list):
        if not out:
            return ""
        return _normalize_generation_output(out[0])
    ...
    # vLLM RequestOutput style: obj.outputs[0].text
...
def _try_call(fn: Any, prompt: str, max_new_tokens: int) -> str:
    """Try common signatures used by our generator wrappers."""
    attempts = [
    ...
    for call in attempts:
        try:
            return _normalize_generation_output(call())
...
def call_generator(generator: Any, prompt: str, max_new_tokens: int) -> str:
    """
    Compatible wrapper for different generator APIs in this project.
    LocalGenerator used by run_local_rewrite_backtrack.py may not expose .generate().
    """
    ...
    for name in ["generate_one", "generate_text", "complete", "generate", "chat"]:
        if hasattr(generator, name):
            ...
            return _try_call(fn, prompt, max_new_tokens)

    # Callable generator.
    if callable(generator):
        return _try_call(generator, prompt, max_new_tokens)

    # Batch methods.
    for name in ["generate_batch", "batch_generate", "generate_many"]:
        if hasattr(generator, name):
            fn = getattr(generator, name)
            attempts = [
...
def main() -> None:
    ...
    parser.add_argument("--accept_policy", default="candidate_only", choices=["candidate_only", "any_number"])
    ...
    for idx, (sid, rs) in enumerate(samples, 1):
        ...
        prompt = build_judge_prompt(rs[0].get("question", ""), rs, ranked)
        ...
        judge_ans = parsed["final_answer"]

        accepted = False
        if judge_ans:
            if args.accept_policy == "any_number":
                accepted = True
            else:
                accepted = judge_ans in candidate_answers
        ...
        row = {
            ...
            "judge_output": output,
            "chosen_candidate": parsed["chosen_candidate"],
